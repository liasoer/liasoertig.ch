...
(function () {
  ...
  /* ---------- Mouse-wheel support for horizontal video carousels ----------
     A plain mouse only has a vertical wheel (deltaY), so without this, a
     .video-grid with more than one video is only reachable by dragging the
     scrollbar or using a trackpad's horizontal swipe — a normal wheel tick
     does nothing, since the grid has no vertical overflow for it to act on.
     Redirect a vertical tick into horizontal scroll of the grid itself,
     whenever there's actually more than one video to scroll to. Real
     horizontal input (trackpad swipe, shift+wheel — deltaX dominant) is
     left alone, since the browser already scrolls that natively. */
  document.addEventListener("wheel", (e) => {
